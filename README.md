...
- 离线演示：工单保存在浏览器本地，不依赖服务器，适合比赛现场保底展示。
- 云端在线：通过 `ws://8.148.79.98:8083/mqtt` 连接 MQTT Broker，与 `Cloud_AI_Engine` 的工单主题联动。
...
这些 PIN 只用于独立演示版。正式部署应由服务端登录接口签发会话，不能在前端保存真实人员密码。
...
- 演示呼叫：`smart_care/ticket/request`
